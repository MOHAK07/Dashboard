...
export function MDAClaimKPI({ className = '' }: MDAClaimKPIProps) {
  ...
  const mdaKPIs = useMemo(() => {
    ...
    if (mdaDatasets.length === 0) {
      return { hasData: false, recoveryPercentage: 0, totalEligible: 0, totalReceived: 0, totalNotReceived: 0 };
    }
    ...
    // Find required columns (case-insensitive with trim)
    const sampleRow = allMDAData[0];
    const columns = Object.keys(sampleRow);
    ...
    const amountNotReceivedColumn = columns.find(col => 
      col.toLowerCase().trim().includes('amount') && 
      col.toLowerCase().trim().includes('not') &&
      col.toLowerCase().trim().includes('received')
    );

    console.log('KPI columns found:', { eligibleAmountColumn, amountReceivedColumn, amountNotReceivedColumn });
    ...
    allMDAData.forEach(row => {
      ...
      const notReceived = amountNotReceivedColumn ? parseIndianAmount(row[amountNotReceivedColumn]) : 0;

      if (eligible >= 0) totalEligible += eligible;
      if (received >= 0) totalReceived += received;
      if (notReceived >= 0) totalNotReceived += notReceived;
    });

    const recoveryPercentage = totalEligible > 0 ? (totalReceived / totalEligible) * 100 : 0;

    console.log('KPI calculations:', { totalEligible, totalReceived, totalNotReceived, recoveryPercentage });
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
            ...
            {mdaKPIs.totalNotReceived > 0 && (
              <div className="flex justify-between">
                <span>Amount Pending:</span>
                <span className="font-medium text-warning-600">{formatAmount(mdaKPIs.totalNotReceived)}</span>
              </div>
            )}
          </div>
        </div>
