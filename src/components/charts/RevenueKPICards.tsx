...
export function RevenueKPICards({ className = "" }: RevenueKPICardsProps) {
  ...
  const revenueData = useMemo((): RevenueData[] => {
    ...
    const fomB2BColumn = columns.find((col) =>
      col.toLowerCase().trim().includes("fom b2b")
    );
    const fomB2CColumn = columns.find((col) =>
      col.toLowerCase().trim().includes("fom b2c")
    );
    const lfomB2CColumn = columns.find((col) =>
      col.toLowerCase().trim().includes("lfom b2c")
    );
    ...
    const processedData: RevenueData[] = allRevenueData
      .map((row) => ({
        ...
        mdaClaimReceived: parseAmount(row[mdaClaimColumn]),
        totalRevenue: parseAmount(row[totalRevenueColumn]),
        fomB2B: parseAmount(row[fomB2BColumn]),
        fomB2C: parseAmount(row[fomB2CColumn]),
        lfomB2C: parseAmount(row[lfomB2CColumn]),
      }))
      .filter((item) => item.month && item.month !== "");
  ...
  return (
    <div className={`card p-0 overflow-hidden ${className}`}>
      <div className="flex flex-col lg:flex-row">
        {/* Left Side: KPIs */}
        <div className="w-full lg:w-2/5 p-6 flex flex-col justify-center items-left text-left">
          <div className="flex items-center space-x-2 mb-2">
            <Calendar className="h-4 w-4 text-primary-600 dark:text-primary-400" />
            <p className="text-sm font-medium text-primary-700 dark:text-primary-300">
              Total Revenue for {displayData.month}
          ...
          <p className="text-3xl font-bold text-primary-900 dark:text-primary-100 mb-10">
            {formatCurrency(displayData.totalRevenue)}
          </p>
        ...
        <div className="hidden lg:flex items-center justify-center">
          <div className="border-l border-gray-200 dark:border-gray-700 h-[14rem]"></div>
        </div>
        ...
        <div className="w-full lg:w-3/5 p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2 pl-5 text-md">
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">
                    B2B Sales
                  </span>
                  <span className="font-semibold text-blue-500 dark:text-blue-400">
                    {formatCurrency(displayData.fomB2B)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">
                  ...
                  <span className="font-semibold text-blue-500 dark:text-blue-400">
                    {formatCurrency(displayData.fomB2C)}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 pl-5 text-md">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-green-500 dark:text-green-400">
                    {formatCurrency(displayData.lfomB2C)}
                  </span>
                </div>
              </div>
            </div>
