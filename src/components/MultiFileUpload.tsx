...
export function MultiFileUpload({
  ...
  const processFile = async (uploadingFile: UploadingFile) => {
    try {
      ...
      setUploadingFiles((prev) =>
        prev.map((f) =>
          f.id === uploadingFile.id ? { ...f, status: "validating" } : f
        )
      );
      ...
      if (extension === "csv") {
        ...
        results = await parseJSON(file);
      } else {
        throw new Error(`Unsupported file type: ${extension}`);
      }

      // --- SIMPLIFIED DATASET CREATION ---
      // The complex validation logic from the old FileParser is removed
      // We now trust the parser and create the dataset directly
      const validationSummary = `${results.data.length} records parsed.`;
      const dataset: Dataset = {
        ...
        status: "valid", // Assume valid, as old validation logic is gone
        rowCount: results.data.length,
        validationSummary: validationSummary,
        color: ColorManager.getDatasetColor(file.name.replace(/\.[^/.]+$/, "")),
        preview: results.data.slice(0, 5),
        dataType: "unknown", // Set to unknown as we no longer have the validation logic
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6">
          ...
          {uploadingFiles.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                Upload Progress (
                {uploadingFiles.filter((f) => f.status === "complete").length}/
